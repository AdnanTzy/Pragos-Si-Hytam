...
document.addEventListener('touchstart', handleJump);
  ...
  // Game state
  let bird = null, pipes = [], score = 0, gameRunning = false, gamePaused = false, gameState = 'ready', frameCount = 0;
  let pipeStartDelay = 0; // 2 second delay before pipes start
  const gravity = 0.5;
  const jump = 7;
  let pipeGap = 150;
  let groundScroll = 0;
  ...
  function enableTouchControls(){
    ...
    canvas.onclick = flap;
    touchControl.onclick = flap;
    touchControl.ontouchstart = (e)=>{ e.preventDefault(); flap(); };
    canvas.addEventListener('touchstart', (e) => { e.preventDefault(); flap(); }, { passive: false });
    gameWrapper.addEventListener('touchstart', (e) => { e.preventDefault(); flap(); }, { passive: false });
  ...
  function loop(){
   ...
   if (gameState === 'playing') {
    ...
    if (ground.complete) {
      ...
      const offset = groundScroll % drawGroundW;
